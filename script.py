...
content = content.replace(old_button_target, new_button_content)

modals = '''
  <!-- Pending Reviews Modal -->
  @if (isPendingModalOpen()) {
  <div class="modal-overlay slide-fade-in" (click)="closePendingModal()">
    <div class="modal-card pending-modal-card bento-card" (click)=".stopPropagation()">
...
content = content.replace('</div>\n', modals, 1) if content.endswith('</div>\n') else content.replace('</div>', modals, 1)
