...
# Extract lines 73 to 152 (0-indexed -> 72 to 152) 
# Note: Line 74 in editor is index 73.
# Let's just find the loop by string matching because line numbers can be off by 1
html = "".join(lines)

loop_regex = re.compile(r'(        @for \(pending of pendingReviews\(\); track pending\.id\) \{.*?        \}\n)', re.DOTALL)
match = loop_regex.search(html)
...
# Now find the whole pendingReviews block
pending_block_regex = re.compile(r'      @if \(pendingReviews\(\)\.length > 0\) \{.*?      \}\n', re.DOTALL)
block_match = pending_block_regex.search(html)
...
html = html[:block_match.start()] + banner + html[block_match.end():]

# Append the modals right before the LAST </div>
# The file ends with:
#   }
#
# </div>
...
if last_div_index != -1:
    html = html[:last_div_index] + modals + html[last_div_index:]
