<!DOCTYPE html>
{{-- Plantilla de factura de Cashier traducida al español (copia publicada de cashier::invoice). --}}
<html lang="es">
<head>
    <meta charset="utf-8">
        ...
        <tr valign="top">
            <td width="180">
                ...
                <p>
                    ...
                    <strong>Fecha:</strong> {{ $invoice->date()->format('d/m/Y') }}<br>

                    @if ($dueDate = $invoice->dueDate())
                        <strong>Vencimiento:</strong> {{ $dueDate->format('d/m/Y') }}<br>
                    @endif
                        ...
                        <tr class="row">
                            <td>
                                ...
                                @if ($item->hasPeriod() && ! $item->periodStartAndEndAreEqual())
                                    <br><span class="dates">
                                        {{ \Carbon\Carbon::createFromTimestampUTC($item->period->start)->format('d/m/Y') }} - {{ \Carbon\Carbon::createFromTimestampUTC($item->period->end)->format('d/m/Y') }}
                                    </span>
                                @endif
                            </td>
                            ...
                            <tr>
                                ...
                                <td colspan="{{ $invoice->hasTax() ? 3 : 2 }}">
                                    @if ($coupon->isPercentage())
                                        {{ $coupon->name() }} ({{ $coupon->percentOff() }}% de descuento)
                                    @else
                                        {{ $coupon->name() }} ({{ $coupon->amountOff() }} de descuento)
                                    @endif
                                </td>
                        ...
                        <tr>
                            ...
                            <td colspan="{{ $invoice->hasTax() ? 3 : 2 }}">
                                @if ($invoice->isTaxExempt())
                                    Exento de impuestos
                                @else
                                    Impuesto a liquidar por inversión del sujeto pasivo
                                @endif
                            </td>
                            <td align="right"></td>
                        </tr>
