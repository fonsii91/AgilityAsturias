...
## ESTADO ACTUAL (solo cambios importantes en el servidor, excluir versiones de la aplicación):
...
- **[Completado 2026-06-11]** Fase 11: Desplegado el rediseño del onboarding de clubes con pago previo (Stripe): el formulario de registro solo crea el lead y el club se aprovisiona al confirmarse el pago vía webhook (`StripeEventListener` + `ClubProvisioningService`), incluyendo el correo de activación al gestor. La migración añadió columnas a `club_leads` (`password`, `stripe_session_id`, `club_id`, `provisioned_at`). **En producción el flujo queda dormido bajo `STRIPE_BYPASS_SUBSCRIPTIONS=true`** (los registros siguen aprovisionando sin pago). Lo pendiente para activar el cobro real está en el checklist de la Sección 11 de `docs/04_Integraciones/suscripciones-stripe.md` (claves live, precios, webhook con su `whsec_`, y estrategia para los clubes existentes ANTES de desactivar el bypass).
- **[Completado 2026-06-11]** Corregido `/root/auto_ssl.sh`: la consulta de slugs excluía al club id 1 (`WHERE id > 1`), por lo que el certificado SAN compartido no incluía `agilityasturias.clubagility.com` y ese subdominio daba error de certificado en los navegadores. Se eliminó el filtro (backup en `/root/auto_ssl.sh.bak-20260611`) y se reemitió el certificado con certbot (`--expand`, ahora 18 nombres, caduca 2026-09-09). Verificado externamente: todos los subdominios de clubes y dominios base validan SSL correctamente.
- **[Completado 2026-06-12]** Fase 12: Desplegado el mecanismo de **periodo de cortesía** y la **sincronización de plan con Stripe**: migraciones `clubs.courtesy_until` y `clubs.plan_locked` aplicadas. Permite migrar los clubes existentes a pago real sin bloquearlos (comando `clubs:grant-courtesy` + columna Cortesía en /admin/clubs) y fijar a un club las funciones de un plan superior al que paga (candado en /admin/clubs, pensado para clubes fundadores). El webhook ahora sincroniza `plan_id` desde el precio facturado en `customer.subscription.created/updated` (⚠️ incluir estos eventos al dar de alta el webhook live). **Todo sigue dormido bajo `STRIPE_BYPASS_SUBSCRIPTIONS=true`**; la activación del cobro sigue el checklist de la Sección 11 de `docs/04_Integraciones/suscripciones-stripe.md`.
- **[Completado 2026-06-13]** Fase 13: Desplegado el blindaje de settings del club y los **módulos ligados a features del plan**: `ClubController::update` ya no reemplaza el JSON de settings completo (merge con whitelist) y los módulos activables (gamificación, fondos, patrocinadores, liga norte) dependen de las features del plan gestionadas en la matriz de `/admin/suscripciones`. Al cambiar de plan (admin o webhook de Stripe), `Club::syncModuleSettingsWithPlan()` apaga los módulos que el nuevo plan no incluye. Se ejecutó `php artisan db:seed --class=SubscriptionSeeder --force` para crear las 4 features de módulos y asignarlas a los planes (verificado vía `/plans-public`). ⚠️ Pendiente de decisión del admin: el club `agilityasturias` (plan profesional) tiene `sponsors_enabled=true` pero `patrocinadores` solo está en élite, y el club `xanastur` (plan `gratis`) no tiene ninguna feature de módulos — en ambos casos los módulos se apagarán cuando su gestor guarde Gestionar Club, salvo que se ajuste la matriz de features o su plan. También desplegado el fix de navegación durante el periodo de cortesía (`/tenant/info` ahora reporta `subscribed=true` con cortesía vigente).
