...
*   **Bitácora Canina RSCE (`settings.rsce_tracker_enabled`):** Activa o desactiva la bitácora personal de competición RSCE de los socios (menú "Bitácoras → Canina (RSCE)"). **Activada por defecto**: los clubes anteriores al switch no tienen la clave en `settings` y la ausencia cuenta como activado (solo un `false` explícito la apaga), tanto en el frontend (menú y guard de ruta) como en la API (middleware `rsce_tracker.enabled`).
*   **Bitácora de Caza RFEC (`settings.rfec_tracker_enabled`):** Ídem con la bitácora personal de competición RFEC (menú "Bitácoras → Caza (RFEC)", middleware `rfec_tracker.enabled`). Misma semántica de activada por defecto.

> [!IMPORTANT] Restricción por plan (implementada en backend)
> La activación de estos módulos está ligada a las **features del plan** contratado (ver [[planes-suscripcion-saas]]), que el administrador global gestiona desde la matriz de funcionalidades de `/admin/suscripciones`. Cada switch corresponde a una feature: Gamificación → `gamificacion`, Provisión de Fondos → `provision-fondos`, Patrocinadores → `patrocinadores`, Liga Norte → `liga-norte`, Bitácora RSCE → `modulo-canina`, Bitácora RFEC → `modulo-caza` (asignación inicial del seeder: Básico solo Liga Norte; Profesional añade Gamificación, Fondos y ambas bitácoras; Élite todas). La regla es estricta para evitar que un club contrate un mes el plan superior y conserve sus módulos al bajar:
